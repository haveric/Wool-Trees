...
public class WoolTrees extends JavaPlugin{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
        if(permHandler != null && (permHandler.has((Player)sender, "wooltrees.adjust") || permHandler.has((Player)sender, "woolTrees.adjust"))){
            if (commandLabel.equalsIgnoreCase("wooltrees") || commandLabel.equalsIgnoreCase("wt")){
                ...
                if(args.length == 0 || (args.length == 1 && args[0].equalsIgnoreCase("help"))){
                    ...
                    if (args[0].equalsIgnoreCase("tree")){
                        msg = "Tree Spawn % is ";
                        val += config.getProperty("Tree Spawn %");
                    } else if (args[0].equalsIgnoreCase("wool")){
                        msg = "Wool Spawn % is ";
                        val += config.getProperty("Wool Spawn %");
                    } else if (args[0].equalsIgnoreCase("big")){
                        msg = "Big Tree Spawn % is ";
                        val += config.getProperty("Big Tree Spawn %");
                    } else if (args[0].equalsIgnoreCase("multi")){
                        msg = "Multicolored Tree Spawn % is ";
                        val += config.getProperty("Multicolored Tree Spawn %");
                    } else if (args[0].equalsIgnoreCase("cost")){
                        msg = "Cost to plant a tree is ";
                        val += config.getProperty("iConomy cost to plant");
                    }        
                    if (msg != ""){
                        sender.sendMessage(ChatColor.DARK_AQUA + "[" + ChatColor.GRAY + "WoolTrees" + ChatColor.DARK_AQUA + "] " + ChatColor.WHITE + msg + ChatColor.DARK_AQUA + val);
                    }
                    ...
                    if (val < 0){
                        ...
                        if (val <= 100){
                            ...
                            multiChance = val;
                            msg = "Multicolored Tree Span % set to: ";
                        } else {
                            err = "Value cannnot be above 100";
                        }
                    } else if (args[0].equalsIgnoreCase("cost")){
    ...
    private void setupConfig(){
        ...
        if (multiChance < 0){
            ...
            multiChance = 0.0;
            log.info(warn+"Multicolored Tree Spawn % below 0. Defaulting to 0");
        } else if (multiChance > 100){
            config.setProperty("Multicolored Tree Spawn %", 100.0);
            multiChance = 100.0;
            log.info(warn+"Multicolored Tree Spawn % above 100. Defaulting to 100");
        }
